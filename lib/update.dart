...
class _myHomePageState extends State<Update> {
  ...
  void updateData(){
    ...
    if (formkey.currentState.validate()) {
      ...
      //NOMBRE
      if (opcion==1) {
        Student stu = Student(currentUserId, valor, surname, ap, mat, mail, num);
        dbHelper.update(stu);
      }
      //APELLIDO PATERNO
      else if (opcion==2) {
        Student stu = Student(currentUserId, name, valor, ap, mat, mail, num);
        dbHelper.update(stu);
      }
      //APELLIDO MATERNO
      else if (opcion==3) {
        Student stu = Student(currentUserId, name, surname, valor, mat, mail, num);
        dbHelper.update(stu);
      }
      //PHONE
      else if (opcion==4) {
        Student stu = Student(currentUserId, name, surname, ap, mat, mail, valor);
        dbHelper.update(stu);
      }
      //EMAIL
      else if (opcion==5) {
        Student stu = Student(currentUserId, name, surname, ap, mat, valor, num);
        dbHelper.update(stu);
      }
      //MATRICULA
      else if (opcion==6) {
        Student stu = Student(currentUserId, name, surname, ap, valor, mail, num);
        dbHelper.update(stu);
      }
      cleanData();
      refreshList();
