...
//creacion de la base de datos
  ...
  _onCreate(Database db, int version) async {
    await db.execute(
        "CREATE TABLE $TABLE ($Id INTEGER PRIMARY KEY, $NAME TEXT, $SURNAME TEXT, $AP TEXT, $MAT TEXT, $MAIL TEXT, $NUM TEXT)");
  }

  Future<List<Student>> getStudents() async {
    var dbClient = await db;
    List<Map> maps = await dbClient.query(TABLE, columns: [Id, NAME, SURNAME, AP,MAT, MAIL, NUM]);
...
/*
  ...
  Future<List<Student>> getStudent(String matriculaBusqueda) async {
    ...
    List<Map> maps = await dbClient.rawQuery(
        "SELECT * FROM $TABLE WHERE $MAT LIKE '%$matriculaBusqueda%'");
    List<Student> student = [];
    if (maps.length > 0) {
      for (int i = 0; i < maps.length; i++) {
        print(Student.fromMap(maps[i]).mat);
